...
use dependencies::sdl2::event::{EventType};
use dependencies::sdl2::video::{GLContext};
use dependencies::sdl2::controller::{GameController};
use dependencies::sdl2::{EventPump, GameControllerSubsystem};
...
pub type Extras = (GLContext, Option<EventPump>, Option<GameControllerSubsystem>, HashMap<i32, (GameController, Player)>);
...
pub fn build_window(window_settings: WindowSettings) -> GfxWindow<Window, Extras> {
    ...
    let controller_subsystem = sdl.game_controller().unwrap_or_else(|err| panic!("Error while making controller sub system: {:?}", err));
    ...
    GfxWindow::new(out_color, out_depth, device, factory, window, (context, Some(event_pump), Some(controller_subsystem), HashMap::new()))
}

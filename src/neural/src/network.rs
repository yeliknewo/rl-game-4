...
impl NeuralNetwork {
    ...
    pub fn set_weights_and_bias(&mut self, mut weights_and_biases: Vec<Vec<Vec<f64>>>) {
        ...
        for layer in &mut self.layers {
            layer.set_weights_and_bias(wab_iter.next().unwrap_or_else(|| panic!("Wab iter next was none")));
        }
    }
}
...
impl NeuralLayer {
    ...
    fn new_random(num_inputs: LayerSizeType, num_outputs: LayerSizeType, min_weight: f64, max_weight: f64, min_bias: f64, max_bias: f64) -> NeuralLayer {
        let mut neurons = vec!();
    ...
    fn set_weights_and_bias(&mut self, mut weights_and_biases: Vec<Vec<f64>>) {
        ...
        for neuron in &mut self.neurons {
            neuron.set_weights_and_bias(wab_iter.next().unwrap_or_else(|| panic!("Wab Iter was different length than neurons")));
        }
    }
}
...
impl Neuron {
    ...
    fn new_random(num_inputs: LayerSizeType, min_weight: f64, max_weight: f64, min_bias: f64, max_bias: f64) -> Neuron {
        let mut weights = vec!();
    ...
    fn fire(&self, inputs: &Vec<f64>) -> f64 {
        ...
        for index in 0..inputs.len() {
            sum += inputs.get(index).unwrap_or_else(|| panic!("Input get Index was none")) * *self.weights.get(index).unwrap_or_else(|| panic!("Weights get Index was none"));
        }
    ...
    fn set_weights_and_bias(&mut self, mut weights_and_bias: Vec<f64>) {
        self.set_bias(weights_and_bias.pop().unwrap_or_else(|| panic!("Weights and Bias was empty")));
